...
class ModelConfigUI:
    ...
    def _update_model_parameters(self):
        ...
        # Create parameters based on model
        if model_version == "google/nano-banana":
            self._create_google_nano_banana_params()
        elif model_version == "bytedance/seedream-4":
            self._create_bytedance_seedream_params()
        elif model_version == "qwen/qwen-image-edit":
            self._create_qwen_image_edit_params()
        elif model_version.startswith("jingyunliang/swinir"):
            self._create_swinir_params()
        elif model_version.startswith("tencentarc/gfpgan"):
            self._create_gfpgan_params()

        # Show all widgets first, then apply conditional visibility
        self.model_params_box.show_all()
        self._apply_initial_conditional_visibility()
    ...
    def get_model_parameters(self):
        ...
        if model_version == "google/nano-banana":
            ...
            # aspect_ratio only sent when size != custom (as per API docs: "used when size ≠ custom")
            if (
            ...
            if self.output_format_combo:
                params["output_format"] = self.output_format_combo.get_active_id()
            # output_quality only sent when format is jpg (as per API docs: "Ignored for PNG")
            if (
            ...
            if self.disable_safety_check:
                params["disable_safety_checker"] = (
                    self.disable_safety_check.get_active()
                )
            ...
            task = (
                self.task_type_combo.get_active_id() if self.task_type_combo else None
            )
